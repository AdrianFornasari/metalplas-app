...
import PendingInlineAssignment, {
  type OperarioOption,
} from '@/components/pending-inline-assignment'
...
type Props = {
  ...
  permitirAsignacionInline?: boolean
  operariosAsignables?: OperarioOption[]
}
...
export default function OfOperacionMobileCard({
  ...
  permitirAsignacionInline = false,
  operariosAsignables = [],
}: Props) {
  ...
  const mostrarAsignacionPendiente =
    permitirAsignacionInline &&
    row.estado_codigo === 1 &&
    operariosAsignables.length > 0

  const mostrarAccionesNormales = !mostrarAsignacionPendiente
  ...
  return (
    <article className="rounded-2xl border border-gray-300 bg-white p-4 text-gray-900 shadow-sm">
      ...
      {mostrarAsignacionPendiente && (
        <PendingInlineAssignment
